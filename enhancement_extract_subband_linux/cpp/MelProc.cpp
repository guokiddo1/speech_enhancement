...
CMelFBAlloc()
{
  MelFB_Window *FirstWin = (MelFB_Window *)calloc(1, sizeof(MelFB_Window));
...
CGammaAlloc()
{
  Gamma_Window *FirstWin = (Gamma_Window *)calloc(1, sizeof(Gamma_Window));
...
//ADD_BY_GC_Gamma
void 
DoGamma(float *SigFFT, Gamma_Window * FirstWin)
{
  Gamma_Window *p1;
  float Sum[WF_MEL_ORDER];
  int i, j;
 ...
 *---------------------------------------------------------------------------*/
void 
InitMelFBwindows(MelFB_Window * FirstWin, float StFreq, float SmplFreq, int FFTLength, int NumChannels, int normalize)
{
  int i, j, k;
  int *centrFreq;
  float freq;
...
//ADD_BY_GC_Gamma
void 
InitGammawindows(Gamma_Window * FirstWin, float StFreq, float SmplFreq, int FFTLength, int NumChannels, int normalize)
{
  int i, j, k;
  float *erb;
  float *centrFreq_fre;
  float freq;
  float start_gamma;
  float fs_per_2_gamma;
  float normFBconst;

  Gamma_Window *p1, *p2;

  centrFreq_fre = (float *) malloc(sizeof(float) * NumChannels);
  erb = (float *) malloc(sizeof(float) * NumChannels);


  /* Constants for calculation */
  start_gamma = HzToERBRate(StFreq);
  fs_per_2_gamma = HzToERBRate(SmplFreq / 2);
  freq = (fs_per_2_gamma-start_gamma)/(NumChannels-1);
  for (i = 0; i < NumChannels; i++) {
    centrFreq_fre[i] =   ERBRateToHz(start_gamma+freq*i) ;
	erb[i] = 1.019 * 24.7 * (4.37 * centrFreq_fre[i]/1000 +1);	
  }
  p1 = FirstWin;

    /*----------------------------------
   * freq. windows 0->NumChannels - 1
   *----------------------------------*/
  for (i = 0; i < NumChannels-1 ; i++) {
    p1->StartingPoint =  (int)centrFreq_fre[i]*FFTLength/SmplFreq;
    p1->Length =  FFTLength/2;;
    p1->Data = (float *) malloc(sizeof(float) * p1->Length);
    if (p1->Data == NULL) {
      fprintf(stderr, "ERROR:   Memory allocation error occured!\r\n");
      exit(0);
    ...
    normFBconst = 0.0;
    for (j = 0; j < p1->Length; j++) {
	  p1->Data[j] = 1.0 / pow((1.0+(j*1.0/FFTLength*SmplFreq-centrFreq_fre[i])/erb[i]*(j*1.0/FFTLength*SmplFreq-centrFreq_fre[i])/erb[i]), 2);
      normFBconst += p1->Data[j];
	}
    if (normalize)
		for (j = 0; j < p1->Length; j++)
			p1->Data[j] /= normFBconst;
    p2 = (Gamma_Window *) malloc(sizeof(Gamma_Window));
    if (p2 == NULL) {
      fprintf(stderr, "ERROR:   Memory allocation error occured!\r\n");
      exit(0);
    }
  ...
  p1->StartingPoint =  (int)centrFreq_fre[NumChannels-1]*FFTLength/SmplFreq;
  p1->Length =  FFTLength/2;;
  p1->Data = (float *) malloc(sizeof(float) * p1->Length);
  if (p1->Data == NULL) {
    fprintf(stderr, "ERROR:   Memory allocation error occured!\r\n");
  ...
  normFBconst = 0.0;
  for (j = 0; j < p1->Length; j++) {
	p1->Data[j] = 1.0 / pow((1.0+(j*1.0/FFTLength*SmplFreq-centrFreq_fre[NumChannels-1])/erb[NumChannels-1]*(j*1.0/FFTLength*SmplFreq-centrFreq_fre[NumChannels-1])/erb[NumChannels-1]), 2);
    normFBconst += p1->Data[j];
  }
  if (normalize)
	for (j = 0; j < p1->Length; j++)
...
//ADD_BY_GC_Gamma
void 
InitGammaIDCTbasis(float **GammaIDCTbasis, Gamma_Window * FirstWin, short melorder, int sampFreq, int FFTLength)
{
  Gamma_Window *p1;
  float centrFreq[WF_MEL_ORDER];
  float deltaFreq[WF_MEL_ORDER];
  ...
  for (i = 0; i < melorder; i++)/* time axis */
    for (j = 0; j < melorder; j++)	/* frequency axis */
      GammaIDCTbasis[i][j] = deltaFreq[j] * cos(PIx2 * i * centrFreq[j] / sampFreq);
}
...
//ADD_BY_GC_Gamma
void 
DoGammaIDCT(float *inData, float **GammaIDCTbasis, int GammaOrder, int timeLength)
{
  int t, f;
  float *outData;
  ...
  for (t = 0; t < timeLength; t++) {
    outData[t] = 0.0;
    for (f = 0; f < GammaOrder; f++) {
      outData[t] += inData[f] * GammaIDCTbasis[t][f];
    }
  }
  for (t = 0; t < timeLength; t++)
    inData[t] = outData[t];
