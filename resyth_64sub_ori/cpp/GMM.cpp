...
GMM::GMM(int dim, int num_mix, int numdata)
{
	DIM=dim;        // This is data dimensionality
	Num_Mix=num_mix;     // Number of GMM mixtures
	NumData=numdata;   // This is the number of data to be used
	Min_samples=2;  // This is the minimum number of samples in each cluster
	Min_weight=0.02; // This is the weight floor for k-mean
	Max_Iter=1000; // This is the maximum number of iterations for k-mean
	threshold=0.0001; // This is the stop criteria for k-mean
	Infinity =1e+32;
	Perturb=0.001; // When a bad cell appears, replace its center by the center
                          // of the most popular cell plus this perturbation
	threshold_EM=0.0001; // stop criteria for EM
		
   	Feature = new float*[NumData];  // Each row is a feature
	for(int i = 0; i < NumData; i++)
		Feature[i] = new float[DIM];
	E_Prob = new float*[Num_Mix]; // This matrix stores Pr(class i|data j) in E ste
	for(int i =0; i < Num_Mix; i++)
		E_Prob[i] = new float[NumData];
	Mixtures = new Mix[Num_Mix];        // Each element is a mixture
	for(int i = 0; i< Num_Mix; i++)
	{
		Mixtures[i].Mean = new float[DIM];
		Mixtures[i].variance = new float[DIM];
	}
}
GMM::~GMM()
{
	...
	for(int i =0; i < Num_Mix; i++)
		delete[] E_Prob[i];
	delete[] E_Prob; // This matrix stores Pr(class i|data j) in E ste
	for(int i = 0; i< Num_Mix; i++)
	{
		delete[] Mixtures[i].Mean;
		delete[] Mixtures[i].variance;
	}
	delete[] Mixtures ;        // Each element is a mixture
}

void GMM::Read_Feature(double *data)   // This function reads features from a file, and store in the feature matrix
{
   int i,j;
